...
import wikipedia
from word import list
import googlemaps
import requests

map_client = googlemaps.Client(key='AIzaSyBVHZn0r4ew-nmCvlI1v27fbfJcKtA31TM')
...
def api_result(resulta):
    try:
        ...
        mape = requests.get(map_url, {"key"  : 'AIzaSyBVHZn0r4ew-nmCvlI1v27fbfJcKtA31TM', "center": adresse, "zoom":15, "size": '400x400', "markers": adresse})
...
def parser(value):
    ...
    resulta = ""
    if valeur == "" or valeur == " ":
        resulta = "le formulaire est vide"
    else :
        liste = valeur.split(" ")
        for word in liste:
            if word not in list:
